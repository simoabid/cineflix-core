...
> Read this when a provider breaks, or when you want to add a _new_ hard provider.
...
- For **Hexa** (Cap.js + vidsrc-class WASM, no enc-dec), see **[HEXA-SCRAPING.md](./HEXA-SCRAPING.md)**.
- For **VidSync** (bro.wasm pure Node proved; Turnstile blocks ship; provider disabled), see **[VIDSYNC-SCRAPING.md](./VIDSYNC-SCRAPING.md)**.

Golden rule learned the hard way: **change one variable at a time, and always
get the _real_ error before theorizing.** Most of the time we lost was from
...
CinePro Core is built on **`@omss/framework`**. You only write _provider_
...
- Discovery **imports every `.ts`** in a provider folder. Any file with
  ...
  startup**. Keep dev/recon scripts _out_ of the providers directory.
...
Right‑click → **Copy → Copy as cURL (bash)** to get _every_ header verbatim
...
> The header that cost us hours: `X-Fingerprint-Lite` — a _constant_ anti‑bot
...
3. **Anti‑bot at the transport layer:** TLS/JA3 + HTTP/2 fingerprint. If the
   browser's _own_ key/headers fail from Node but work in Chrome, and an
...
- Works from Node with browser identity → it was app‑layer (header/token). Fix
  in code.
- Fails from Node even with browser identity, _but_ an unsigned endpoint on the
...
- Put failures in `diagnostics` _and_ `this.console.log`.
...
2. **Discover what it touches**: `grep 'imports.wbg.__wbg_'` in the glue lists
   every host function the wasm calls. That's the _exact_ set of browser globals
...
5. **Only call the pure‑compute exports** from Node (e.g. key derivation +
   decryption). Reimplement the _networking/signing_ in TS yourself — it's
...
_plausible and stable_ (the fingerprint gates that they exist, not their exact
...
- `403 {"error":"no sources found"}` (decoy) → **missing `X-Fingerprint-Lite`**
  constant + wrong `Sec-Fetch-Site`. This was _the_ blocker; proven by the
...
4. **Proxy must rewrite HLS** → never put whole CDN hosts in `streamPatterns`
   (that made master `.m3u8` take the _stream_ path and skip rewrite). Stream
...
5. **Host-without-scheme URIs** → Oxygen masters emit
   ...
   _path under the master host_. Patch: only when `host.tld/path…` (requires
...
7. **Hydrogen “decoy” that isn’t** → media playlists often list
   ...
   (`bew.jpg` / `bex.html` / `bey.js`) _are_ real decoys. Distinguish by
...
- **Header/token/constant changed** (most common, ~1‑line): _fix in place_.
  Update `vidsrcClient.ts`. Minutes.
- **New WASM / new env checks**: _investigate + patch_. Re‑download assets,
  extend the shim, re‑test. Tens of minutes.
- **Endpoint/domain/flow changed** but still an SPA+WASM design: _partial
  rebuild_ of `vidsrcClient.ts` using this book's §2 method. An hour‑ish.
- **Whole site re‑architected** (e.g. moves to server‑side TLS gating, or a
  fundamentally different player): _rebuild from scratch_ — but you now have the
...
_Written from the vidsrc build; §6b added from the vidking build (2026-07).
Hexa native path (Cap + WASM) documented separately in [HEXA-SCRAPING.md](./HEXA-SCRAPING.md).
The specifics will age; the method won't._
