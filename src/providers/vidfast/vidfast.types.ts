...
/**
 * result of GET enc-dec.app/api/enc-vidfast?text=<pageToken>.
 ...
 * - `token`   : csrf token that must be sent as X-CSRF-Token on the vidfast
 *               server/stream requests.
 */
export interface VidfastHandshake {
    servers: string;
    stream: string;
    token: string;
}
...
/**
 * one entry from the decrypted server list (dec-vidfast of the servers blob).
 ...
 * field names differ across vidfast builds, so all common aliases are covered.
 */
export interface VidfastTrack {
    url?: string;
    file?: string;
    lang?: string;
    language?: string;
 ...
 */
export interface VidfastStreamFile {
    url?: string;
    file?: string;
    type?: string;
    quality?: string;
...
/**
 * the final decrypted stream object (dec-vidfast of a per-server stream blob).
 *
 ...
 * The normalizer in vidfast.ts reads whichever of these is present.
 */
export interface VidfastDecryptedStream {
    url?: string;
    file?: string;
    type?: string;
    quality?: string;
    sources?: VidfastStreamFile[];
    qualities?: Record<string, { url?: string; file?: string; type?: string }>;
    tracks?: VidfastTrack[];
    subtitles?: VidfastTrack[];
    captions?: VidfastTrack[];
}
