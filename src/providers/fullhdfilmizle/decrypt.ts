import type { PackerParams } from './fullhdfilmizle.types.js';

export function rtt(str: string): string {
...
export function decodeAtom(e: string): string {
...
export function decodeDeanEdwards(params: PackerParams): string {
    ...
    const encodeBase = (num: number): string => {
        ...
        const char = num % radix;
        const suffix =
            char > 35 ? String.fromCharCode(char + 29) : char.toString(36);
